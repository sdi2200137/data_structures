...
void map_insert(Map map, Pointer key, Pointer value) {
	...
	MapNode node = NULL;
	uint pos;
	uint hashpos = map->hash_function(key) % map->capacity;
	uint e = 0;
	...
	//μπορεί να έχουμε βρει empty αλλά το στοιχείο να βρίσκεται σε καποια από τις επόμενες γειτονικές θέσεις
	if(!already_in_map){
		if(e < NEIGHBOURS){
			int count = 0;
			for(uint temp_pos = map->hash_function(key) % map->capacity;		// ξεκινώντας από τη θέση που κάνει hash το key
				count <= NEIGHBOURS;						// αν φτάσουμε σε EMPTY σταματάμε
				temp_pos = (temp_pos + 1) % map->capacity) {
				if(map->array[temp_pos].state == EMPTY) {
					count ++;
					continue;
				}
				else if (map->compare(map->array[temp_pos].key, key) == 0) {
					already_in_map = true;
					node = &map->array[temp_pos];						// βρήκαμε το key, το ζευγάρι θα μπει αναγκαστικά εδώ (ακόμα και αν είχαμε προηγουμένως βρει DELETED θέση)
					break;											// και δε χρειάζεται να συνεχίζουμε την αναζήτηση.
				}
				count ++;
			}
	...
	if (already_in_map) {
		...
		//αν το e βρισκεται σε γειτονικη θέση τοποθετούμε το στοιχείο εκεί
		if(e <= NEIGHBOURS){
			node = &map->array[pos];
		}
		else {
			//σε διαφορετική περίπτωση αρχίζουμε από το προηγούμενο του e
			//και προσπθούμε να βρούμε στοιχείο που μπορεί να τοποθετηθεί στην κενή θέση 
			uint i = (e - 1) % map->capacity;
			//μεχρι την επόμενη θέση απο αυτή που κανει hash το στοιχείο που θέλουμε να τοποθετήσουμε 
			while(i >= 1){
				MapNode node2 = &map->array[hashpos + i];
				if(node2->state == OCCUPIED){
					uint haspos_i = map->hash_function(node2->key) % map->capacity;
					//αν το στοιχείο είναι γειτονικό της κενής θέσης
					if((e - haspos_i < NEIGHBOURS) && (haspos_i - e < NEIGHBOURS)){
						//μετακινούμε το γειτονικό στοιχείο στην κενή θέση και μετατρέπουμε την τωρινή του θέση σε κενή
						MapNode temp =  &map->array[hashpos + i];
						node2 = &map->array[pos];
						...
						temp->key = NULL;
						temp->value = NULL;
						//επόμενη κενή θέση στο στοιχείο που μετακινήθηκε
						e = i;
				...
				//αν το e έγινε γειτονικό της θέσης που κανει hash το στοιχείο που θέλουμε να τοποθετήσουμε τότε το βάζουμε εκεί
				if(e <= NEIGHBOURS){
					node = &map->array[hashpos + e];
					break;
				}
				i = (i - 1) % map->capacity;
			}
			...
			if(node == NULL){
				//αν δεν έχει τοποθετηθεί κάπου το στοιχείο κάνουμε rehash και παλι insert το ίδιο στοιχείο
				rehash(map);
...
MapNode map_next(Map map, MapNode node) {
	...
	//αρχίζουμε από την επομενη
	int next_in_array = i + 1;

	for (int j = next_in_array; j < map->capacity; j++)
