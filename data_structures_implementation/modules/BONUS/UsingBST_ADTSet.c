...
static SetNode node_create(Pointer value) {
	...
	// node->left = NULL;
	// node->right = NULL;
    node->tree = rectree_create(value, REC_TREE_EMPTY, REC_TREE_EMPTY);
	//node->value = value;
	return node;
}
...
static SetNode node_find_equal(SetNode node, CompareFunc compare, Pointer value) {
	...
	//
	int compare_res = compare(value, rectree_value(node->tree));			// αποθήκευση για να μην καλέσουμε την compare 2 φορές
	if (compare_res == 0)									// value ισοδύναμη της node->value, βρήκαμε τον κόμβο
		...
		return node_find_equal(rectree_left(node->tree), compare, value);
	else													// value > node->value, ο κόμβος που ψάχνουμε είνια στο δεξιό υποδέντρο
		return node_find_equal(rectree_right(node->tree), compare, value);
}

// Επιστρέφει τον μικρότερο κόμβο του υποδέντρου με ρίζα node

static SetNode node_find_min(SetNode node) {
	return node != NULL && rectree_left(node->tree) != NULL
		? node_find_min(rectree_left(node->tree))				// Υπάρχει αριστερό υποδέντρο, η μικρότερη τιμή βρίσκεται εκεί
...
static SetNode node_find_max(SetNode node) {
	return node != NULL && rectree_right(node->tree) != NULL
		? node_find_max(rectree_right(node->tree))			// Υπάρχει δεξί υποδέντρο, η μεγαλύτερη τιμή βρίσκεται εκεί
...
static SetNode node_find_previous(SetNode node, CompareFunc compare, SetNode target) {
	if (node == target) {
		...
		return node_find_max(rectree_left(node->tree));

	} else if (compare(rectree_value(target->tree), rectree_value(node->tree)) < 0) {
		// Ο target είναι στο αριστερό υποδέντρο, οπότε και ο προηγούμενός του είναι εκεί.
		return node_find_previous(rectree_left(node->tree), compare, target);
		...
		SetNode res = node_find_previous(rectree_right(node->tree), compare, target);
...
static SetNode node_find_next(SetNode node, CompareFunc compare, SetNode target) {
	if (node == target) {
		...
		return node_find_min(rectree_right(node->tree));

	} else if (compare(rectree_value(target->tree), rectree_value(node->tree)) > 0) {
		// Ο target είναι στο δεξί υποδέντρο, οπότε και ο επόμενός του είναι εκεί.
		return node_find_next(rectree_right(node->tree), compare, target);
		...
		SetNode res = node_find_next(rectree_left(node->tree), compare, target);
		return res != NULL ? res : node;
	}
}
